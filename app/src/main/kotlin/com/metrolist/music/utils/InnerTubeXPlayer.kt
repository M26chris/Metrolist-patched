 ...
 *
 * This object manages YouTube playback stream resolution with intelligent client fallback:
 * - Tries clients in priority order (visionOS, ANDROID, WEB_CREATOR, TVHTML5, WEB_REMIX)
 * - Tracks failed clients per-video with 5-minute TTL
 * - Excludes failed clients from next extraction attempt
 * - Provides full diagnostics for debugging playback issues
 *
 * The key innovation: per-video failure tracking prevents retrying capped clients
 * (like WEB_REMIX) immediately. Instead, uncapped clients are tried first on the
 * next playback, and capped clients are only used as a fallback.
 */
object InnerTubeXPlayer {
    private const val TAG = "InnerTubeXPlayer"
    private const val STREAM_CLIENT_FAILURE_TTL_MS = 5 * 60 * 1000L  // 5 minutes
     ...
     *
     * This is the main entry point for playback. It:
     * 1. Gets the set of failed clients for this video (if any)
     * 2. Passes them to the extractor so they're excluded from selection
     * 3. Tries clients in priority order until one succeeds
     * 4. Returns playback data with full diagnostics
     *
     * If extraction fails with a 403 and we haven't exhausted all clients,
     * the caller should call [markStreamClientFailed] and retry.
     */
    suspend fun playerResponseForPlayback(
        ...
        try {
            ...
            if (excludedClients.isNotEmpty()) {
                Timber.tag(TAG).i(
                    "Playback: excluding clients for $videoId: $excludedClients"
                )
            }
            val stream =
                requireNotNull(
            ...
            Timber.tag(TAG).d(
                "Playback: client=${stream.clientName}, url_range=${stream.audioUrl.take(50)}..., expires_in=${stream.expiresAt}"
            )
            ...
            Timber.tag(TAG).w(
                error,
                "Playback: extraction failed: reason=${error.reason}"
            )
            Result.failure(
            ...
            Timber.tag(TAG).e(error, "Playback: unexpected error")
            Result.failure(error)
        }

    /**
     * Mark a client as failed for a specific video.
     *
     * This should be called when a playback attempt fails with 403 or similar.
     * The client will be excluded from the next extraction attempt for the same video,
     * and will expire after [STREAM_CLIENT_FAILURE_TTL_MS].
     *
     * Example: When WEB_REMIX hits the 1 MiB quota on a long track, call this with
     * clientName="WEB_REMIX" to prevent immediately retrying the same capped client.
     */
    internal fun markStreamClientFailed(
        ...
        Timber.tag(TAG).w(
            "Marked client as failed: videoId=$videoId, client=$clientName"
        )
    }

    fun clearStreamClientFailures() {
        streamClientFailures.clear()
        Timber.tag(TAG).d("Cleared all stream client failures")
    }

    suspend fun refreshAfterStreamRejection(): Boolean = bundle().cipherService.refreshAfterStreamRejection()

    /**
     * Get the set of clients that have failed for a given video.
     *
     * Returns an empty set if no clients have failed, or if the TTL has expired.
     * The TTL is [STREAM_CLIENT_FAILURE_TTL_MS], so after 5 minutes, a failed
     * client can be retried again.
     */
    internal fun failedStreamClients(
        ...
        if ((nowMs - failures.failedAtMs) !in 0 until STREAM_CLIENT_FAILURE_TTL_MS) {
            ...
            Timber.tag(TAG).d("Failed clients cache expired for $videoId")
            return emptySet()
        }
    ...
    /**
     * Complete playback data including stream URL, headers, and client info.
     */
    class PlaybackData(
        ...
        object : YtConfigParser by this {
            override suspend fun fetchConfig(
                ...
                try {
                    ...
                    Timber.tag(TAG).w("Falling back to embedded config for $videoId")
